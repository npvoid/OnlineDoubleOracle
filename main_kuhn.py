...
from kuhn_poker_exploitability.online_learning_kuhn import online_learning_kuhn
...
def main(row_num_pol_init, col_num_pol_init, n_brs, n_rounds, eps, lp_nash_iter, refractory, whole_time_average, ref_func=None):
    ...
    for i in range(n_rounds):
        print(f'round: {i}')
        exp_history_i = \
            online_learning_kuhn(row_num_pol_init, col_num_pol_init, n_brs, eps, lp_nash_iter=lp_nash_iter, refractory=refractory,
        ...
        save_dir = Path(__file__).parent / "results_kuhn" / f"kuhn_poker-online_oracle-vs-online_oracle--n_rounds-{n_rounds}--n_brs-{n_brs}--time-{dt_string}"
    ...
    print(f'Average Time per round for Online-Oracle vs Online-Oracle in Game Kuhn Poker are {avg_time_per_round} seconds')
    plt_experiments(n_rounds, n_brs, 'kuhn', dt_string)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--n_brs', '-ni', help='number of best responses', type=int, default=30)
